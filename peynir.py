...
def db_file_check():
    if not os.path.isfile(db_dir+db_file):
        try:
            text_formatting(_("Database could'nt found. Getting from server"),0)
            retrieve(db_dir,mirror+db_file,db_dir+db_file)
            text_formatting(_("Database successfully updated."),0)
        except:
            text_formatting(_("Database couldn't updated."),0)
            sys.exit(1)
...
def uptodate(xml_source,type_file):
    if type_file == "web":
       try:
           ...
           text_formatting(_("-> Peynir also updating pacman repositories"),1)
           devnull = open('/dev/null', 'w')
           try:
               subprocess.Popen("pacman -Sy --noconfirm ", shell=True, stdout=devnull).wait()
           except:
               text_formatting(_("There is a problem with pacman sync repository, If you continue to upgrade, I can get some error"),1) 
       except:
           text_formatting(_("Database couldn't updated."),0)
           sys.exit(1)
    else:
        try:
            ...
            text_formatting(_("There is a problem with xml file"),0)
            sys.exit(1)
...
def retrieve(place,url,file):
    ...
    try:
        ...
        text_formatting(file + _(" could not retrivied from mirror"),0)
        sys.exit(1)
    fp = open(file, 'wb')
    fp.write(data)
    fp.close()
...
def sync_repo():
    text_formatting(_(":: Local database synchronizing with server"),0)
    if not os.path.isfile(db_dir+db_file) or uptodate(db_dir+db_file,"local") != uptodate(mirror+db_file,"web"):
       ...
       text_formatting(_("Database already updated."),0)
       #sys.exit(1)
...
def conflict(source):
    ...
    text_formatting(_(":: Check for conflicts..."),0)
    conflictcount = len(root[1])
    conflicts = root[1]
    if conflictcount > 0:
       text_formatting(str(conflictcount) + _(" conflicts have found."),1)
       text_formatting(_("Following suprapackage(s) will remove. "),0)
       for conf in root[1]:
           try:
               text_formatting(conf.text + " ==> " + get_description(conf.text),1)
           except:
               text_formatting(conf.text + _(" ==> There is no description for this package"),1)
       ...
       if answer == _("Y"):
           for conf in root[1]:
               ...
               if package_check(package):
                   ...
                   text_formatting(package + _(" is not installed"),0)
       else:
           sys.exit(_("Suprapackage coulnd't installed."))
    else:
       text_formatting(_("There is a no conflict"),1)
    ...
    if dependcount > 0:
        for dep in dependencies:
            if os.path.isfile(sprpckg_dir + dep.text +".xml"):
                ...
                text_formatting(_("There is no file to remove local dependencies"),1)
            
def dependencies(source,action):
    text_formatting(_(":: Resolving dependencies.."),0)
    tree = etree.parse(sprpckg_dir+source+".xml")
    root = tree.getroot()
    ...
    dependencies = root[2]
    if dependcount > 0:
       text_formatting(str(dependcount) + _(" dependencies have found."),1)
       #Tamamen kaldırma işlemi için ....
       if action == "remove":
           text_formatting(_("Following suprapackage(s) will remove. "),1 )
           for dep in root[2]:
               try:
                   text_formatting(dep.text + " ==> " + get_description(dep.text),1)
               except:
                   text_formatting(dep.text + _(" ==> There is no description for this package"),1)
           answer = input(_("Are you want to remove these packages (Y/N): "))
           answer = answer.upper()
           if answer == _("Y"):
               for dep in root[2]:
           ...
           text_formatting(_("Following suprapackage(s) will install. "),0)
           for dep in root[2]:
               try:
                   text_formatting(dep.text + " ==> " + get_description(dep.text),1)
               except:
           ...
           if answer == _("Y"):
               for dep in root[2]:
                   ...
                   if not package_check(package):
                       ...
                       text_formatting(":: " + package + _(" is already installed"),0)
       ...
       text_formatting(_("There is a no dependencies"),1)
...
def install(source, place):
    ...
    if place == "local":
       ...
       tree = etree.parse(sprpckg_dir+source+".xml")
    text_formatting(":: " + source + _(" suprapackage is installing"),0)
    conflict(source)
    dependencies(source,"")
    root = tree.getroot()
    steps = len(root[3])
    text_formatting(str(steps) + _(" step(s) will executed."),0)
    counter = 1
    for step in root[3]:
       ...
       try:
           if action_type == "1":
               ...
               text_formatting("-> " + str(len(step)-1) + _(" substep(s) will execute for this step"),1)
           ...
           text_formatting(_("Error occureed in step of ") + step,0)
           sys.exit(1)

def remove(source, rmv_type, dep_source):
    if not package_check(source):
       sys.exit(_("This suprapackage is not installed."))
    text_formatting(":: " + source + _(" suprapackage is removing"),0)
    ...
    if (len(local_dep) == 1 and not "" in local_dep and dep_source in local_dep) or rmv_type == "upgrade":
       ...
       text_formatting(_("Following suprapackages requies ") + source + _(" suprapackage"),0)
       ...
       if rmv_type != "skip":
           ...
           text_formatting(_("Skipping removing ") + source + _(" suprapackage and continue remove process"),0) 
    else:
        remove_action(source, rmv_type)
...
def remove_action(source, rmv_type):        
    ...
    try:
       ...
       text_formatting(_("Local package couldn't found"),0)
       sys.exit(1)
    root = tree.getroot()
    steps = len(root[3])
    counter = 1
    for step in reversed(root[3]):
       ...
       try:
           if action_type == "1":
               if remove_tag == "skip":
                   text_formatting(_("-> Skipping this step for remove action"),1)
               else:
                   package = step.text
               ...
               if remove_tag == "skip":
                   ...
                   try:
                       ...
                       text_formatting(_("There is no defined action for this step"),1)
           ...
           text_formatting(_("Error occured when remove suprapackage"),0) #bu ne bu, bu ne, ne hatası bu :)
           sys.exit(1)
...
def upgrade():
    ...
    for fname in dirlist:
       if upcontrol(fname[:-4],"local") != upcontrol(fname[:-4],"repo"):
           ...
           counter = counter + 1
    if len(up_list) == 0:
        text_formatting(_("There is nothing to do"),0)
    else:
        text_formatting(_("Following suprapackage(s) will upgrade"),0)
        for up in up_list[0:]:
            text_formatting(up + " ==> " + get_description(up),1)
        devnull = open('/dev/null', 'w')
        try:
            text_formatting(_("-> Peynir first upgrade your system via pacman"),1)
            subprocess.Popen("pacman -Su --noconfirm ", shell=True, stdout=devnull).wait()
        except:
            text_formatting(_("There is a problem with upgrade the system via pacman, If you continue to upgrade, I can get some error"),1)
        for up in up_list[0:]:
            text_formatting(up + _(" suprapackage is upgrading ..."),0)
            #answer ı nasıl aktaracağız bakalım ???
            try:
                text_formatting(_("-> Retrieve updated suprapackage from server"),1)
                retrieve(sprpckg_dir,mirror + up +".xml",sprpckg_dir + up + "_bck" +".xml")
            except:
                text_formatting(_("-> There is a problem with getting updated suprapackage from server"),1)
            try:
                tree = etree.parse(sprpckg_dir + up +".xml")
                root = tree.getroot()
                local_dep = root[2].attrib['local']
                text_formatting(_("-> Transision local dependencies from old one to updated one"),1)
                modify_add(sprpckg_dir + up + "_bck"+".xml","<Dependencies","'>"," " + local_dep,"previous")
            except:
                text_formatting(_("-> Error from transision local dependencies"))             
            remove(up,"upgrade","")
    ...
    if len(sys.argv) == 1:
        ...
        if sys.argv[1] == "-S":
            ...
            if not package_check(package[:-4]):
                ...
                text_formatting(_(":: This suprapackage already installed in your system."),0)
        else:
            text_formatting(_("Invalid argument: ") + sys.argv[1],0)
            sys.exit(1)
    else:
        sys.stderr.write(_('Usage: peynir [command] [suprapackage] \n Commands: \n        -S Install suprapackage \n        -U Install local suprapackage \n        -R Remove suprapackage \n        -Rs Remove suprapackage and its dependencies \n        -Sy Update repository \n        -Su Upgrade the system \n        -Ss Search suprapackege in repository \n        -Qs Search suprapackege in local \n        -h Display the help screen \n'))
        sys.exit(1) 
